...
internals.Err.prototype.getPath = function () {
    ...
    var recursePath = function (recursedItem) {

        var reachedItem = Hoek.reach(recursedItem, 'context.reason.0');
        if (reachedItem && reachedItem.context) {
            return recursePath(reachedItem);
        }

        return recursedItem.path;
    };

    return recursePath(self) || self.path || self.context.key;

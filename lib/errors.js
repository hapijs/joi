...
internals.serializer = function () {
    ...
    return function (key, value) {
        ...
        if (value === Infinity || value === -Infinity ||
            Number.isNaN(value) || typeof value === 'function') {
            return '[' + value.toString() + ']';
        }
...
internals.annotate = function () {
    ...
    let message = internals.safeStringify(obj, 2)
        .replace(/_\$key\$_([, \d]+)_\$end\$_\"/g, ($0, $1) => {
            ...
            return '\n' + $2 + ' \u001b[31m[' + $1 + ']\u001b[0m';
        }).replace(/\[(NaN|Symbol.*|-?Infinity|function.*)\]/g, function ($0, $1) {
            switch ($1) {
                case 'NaN':
                    return NaN;
                case 'Infinity':
            ...
            /* $lab:coverage:off$ */
            if ($1.indexOf('Symbol') === 0) {
                 // find symbol's key, e.g. Symbol(key)
                var key = $1.split('(')[1].slice(0, -1);
                return Symbol(key);
            }
            /* $lab:coverage:on$ */

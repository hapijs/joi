    ...
    return this._test('includes', inclusions, function (value, state, options) {
        ...
        for (var v = 0, vl = value.length; v < vl; ++v) {
            ...
            for (var i = 0, il = inclusions.length; i < il; ++i) {
                ...
                if (il === 1) {
                    return Errors.create('array.includes-single', { value: item, pos: v, reason: result.errors }, { key: state.key, path: localState.path }, options);
                }
            }

            if (!isValid) {
                return Errors.create('array.includes', { value: item, pos: v }, { key: state.key, path: localState.path }, options);
            }
        }
    ...
    return this._test('excludes', exclusions, function (value, state, options) {
        ...
        for (var v = 0, vl = value.length; v < vl; ++v) {
            ...
            for (var i = 0, il = exclusions.length; i < il; ++i) {
                ...
                if (!result.errors) {
                    return Errors.create('array.excludes', { value: item, pos: v }, { key: state.key, path: localState.path }, options);
                }
            }
        }

...
internals.Array.prototype._base = function (value, state, options) {
    ...
    if (typeof value === 'string' &&
        isNaN(value) &&                     // Check with isNaN, because JSON.parse converts number string to number
        options.convert) {

        try {
            var converted = JSON.parse(value);
            result.value = Array.isArray(converted) ? converted : [converted];
        }
        catch (e) {
        }
    }
    ...
    return this._test('includes', inclusions, function (value, state, options) {
        ...
        for (var v = 0, vl = value.length; v < vl; ++v) {
            ...
            for (var i = 0, il = inclusions.length; i < il; ++i) {
                var result = inclusions[i]._validate(item, { key: v, path: (state.path ? state.path + '.' : '') + v }, options);
                if (!result.errors) {

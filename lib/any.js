...
internals.Any.prototype._validate = function (value, state, options) {
    ...
    state = state || { parent: null, key: '', path: '' };
    ...
    var process = function () {
        ...
        for (var i = 0, il = self._dependencies.length; i < il; ++i) {
            var err = self._dependencies[i].func.call(self, value, state, options);
            if (err) {
        ...
        if (self._invalids.has(value, self._flags.insensitive)) {
            result.errors.push(Errors.create(value === '' ? 'any.empty' : 'any.invalid', { value: (value === '' ? '' : self._invalids.key(value)) }, state, options));
            if (options.abortEarly) {
                return result;
            }
        }
        ...
        if (options.convert &&
            ...
            if (original !== value) {
                ...
                if (self._invalids.has(value, self._flags.insensitive)) {
                    result.errors.push(Errors.create('any.invalid', { value: self._invalids.key(value) }, state, options));
                    if (options.abortEarly) {
                        return result;
                    }
                }
        ...
        if (self._flags.allowOnly) {
            result.errors.push(Errors.create('any.validate.allowOnly', { value: self._valids.toString(false) }, state, options));
            if (options.abortEarly) {
                return result;
            }
        }
        ...
        for (i = 0, il = self._tests.length; i < il; ++i) {
            ...
            var err = test.func.call(self, value, state, options);
            if (err) {
    ...
    var finish = function () {
        ...
        if (!result.errors.length &&
            state.parent &&
            ((options.modify && state.parent.hasOwnProperty(state.key) && isConverted) || (value === undefined && self._flags.default !== undefined))) {

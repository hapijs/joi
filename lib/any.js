...
internals.Any.prototype.recursive = function () {

    var obj = this.clone();
    obj._flags.recursive = true;
    return obj;
}
...
internals.Any.prototype._validate = function (value, state, options, reference) {
    ...
    if (self._flags.recursive) {
        state.recursiveSchema = self;
    } else if (self._type === "recurse") {
        //console.log(state.recursiveSchema);
        Hoek.assert(state.recursiveSchema, 'recursive() must be called on ancestor schema when using recurse()');
        return state.recursiveSchema._validate(value, state, options);
    }

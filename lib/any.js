...
internals.Any.prototype.when = function () {

    var args = Utils.flatten(Array.prototype.slice.call(arguments));

    var peer = args[0];
    var peerValue = args[1];
    var inclusions = args.slice(2);

    Utils.assert(typeof peer === 'string', 'peer must be string');
    Utils.assert(typeof peerValue !== 'undefined', 'peer value must be defined');
    Utils.assert(typeof inclusions.length !== 0, 'value must be defined');

    this._dependency('when', args, function (value, state, options) {

        if (!state.parent) {
            return Errors.create('any.when.parent', null, state, options);
        }
        ...
        var peerErr = peerValue._validate(state.parent[peer], { parent: state.parent, key: peer, path: peer, renamed: state.renamed }, options);
        if (peerErr) {
            return Errors.create('any.when.peer.value', { value: peerValue, pos: peer, reason: err }, state, options);
        }

        for (var i = 0, il = inclusions.length; i < il; ++i) {
            var inclusion = inclusions[i];
            var err = inclusion._validate(value, state, options);
            if (err) {
                return Errors.create('any.when.value', { value: value }, state, options);
            }
        }

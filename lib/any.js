...
internals.Any.prototype._validate = function (value, state, options) {
    ...
    var peerValidators = ['with','without','or','xor'];

    var applyPeerValidators = function() {
        for (var i = 0, il = self._validators.length; i < il; ++i) {
            if(!~peerValidators.indexOf(self._validators[i].name)) {
                continue;
            }
            var err = self._validators[i].func(value, state, options);
            if (err) {
                Utils.assert(typeof err === 'object', 'oops');
                if (Array.isArray(err)) { errors = errors.concat(err); }
                else {
                    errors.push(err);
                }

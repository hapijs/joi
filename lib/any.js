...
internals.Any.prototype.custom = function (fn) {

    Hoek.assert(typeof fn === 'function', 'Custom must have a function');

    return this._test('custom', {}, function(value, state, options) {
        var error = fn(value);
        if (error) {
            options.language.custom = error;
            return Errors.create('custom', {}, state, options);
        }
        return null;
    });
};

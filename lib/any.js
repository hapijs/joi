...
internals.Any.prototype.matches = function () {
    ...
    this._test('matches', peers, function (value, state, options) {

        if (!state.parent) {
            return internals.Any.error('any.matches.parent', null, state);
        }
        ...
        for (var i = 0, il = peers.length; i < il; ++i) {
            if (!state.parent.hasOwnProperty(peers[i]) ||
                ...
                return internals.Any.error('any.matches.peer', { value: peers[i] }, state);
            }

            if (state.parent[peers[i]] !== value) {
                return internals.Any.error('any.matches.match', { value: peers[i] }, state);
            }
        }

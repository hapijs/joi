...
internals.Any.prototype.type = function (type) {
    Hoek.assert(type, type + ' is not defined');

    var obj = this.clone();
    obj._allowedType = type;
    return obj;
};
...
internals.Any.prototype._validate = function (value, state, options) {
    ...
    var process = function () {
        ...
        if (self._allowedType && (!(value instanceof self._allowedType))) {
            errors.push(Errors.create('any.type.invalid', { type: self._allowedType, value: value }, state, options));
            if (options.abortEarly) {
                return errors;
            }
        }

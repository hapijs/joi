...
internals.Any.prototype.when = function () {
    ...
    Utils.assert(args.length === 2, 'Arguments must contain exactly two arguments');

    var peer = args[0];
    var peerValue = args[1];

    Utils.assert(typeof peer === 'string', 'peer must be a string');
    Utils.assert(peerValue && typeof peerValue._validate === 'function', 'peer value must be defined');

    this._test('when', peer, function (value, state, options) {
        if (state.parent &&
            state.parent.hasOwnProperty(peer)) {

            var peerErr = peerValue._validate(state.parent[peer], {
                parent: state.parent,
                key: peer,
                path: peer,
            ...
            if (peerErr) {
                return Errors.create(
                    'any.when.peer.value',
                    { value: peerValue, pos: peer, reason: peerErr },
                    state,
                    options
                );
            }
        ...
        return Errors.create('object.when', {}, state, options);
    });

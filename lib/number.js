...
internals.decimalPlaces = function (num) {

    const places = num.toString().match(internals.precisionRx);
...
internals.Number.prototype.multiple = function (base) {
    ...
    return this._test('multiple', base, (value, state, options) => {
        ...
        const valueDecimals = internals.decimalPlaces(value);
        const divisorDecimals = internals.decimalPlaces(divisor);
        const mult = Math.pow(10, valueDecimals > divisorDecimals ? valueDecimals : divisorDecimals);

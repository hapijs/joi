        ...
        for (var j = 0, jl = items.length; j < jl; ++j) {
            ...
            if (!schema) {
                ...
                if (failed) {
                    ...
                    // if the item is not the last element in the array
                    // keep checking other attributes
                    if (j < jl - 1) {

                        continue;

                    } else {
                        // if by the last element there are no failures, assign the
                        // then target as the current schema
                       schema = item.then;
                    }
                }

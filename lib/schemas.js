...
internals.rule = Joi.object({
    ...
    args: Joi.array().items(
        ...
        Joi.object({
            ...
            ref: Joi.boolean(),
            assert: Joi.alternatives([Joi.func(), Joi.object().schema()]).when('ref', { is: true, then: Joi.required(), otherwise: Joi.forbidden() }),
            normalize: Joi.func().when('ref', { is: true, otherwise: Joi.forbidden() }),
            message: Joi.string().when('assert', { is: Joi.object(), then: Joi.forbidden(), otherwise: Joi.required() })
        })
    ),
    multi: Joi.boolean(),
    convert: Joi.boolean()
...
exports.extension = Joi.object({
    ...
    initialize: Joi.func().arity(0),
    messages: [Joi.object(), Joi.string()],
    modify: Joi.func().arity(3),
    rebuild: Joi.func().arity(0),

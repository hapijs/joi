    ...
    return function (ctorArgs) {
        ...
        if (extension.rules) {
            for (const rule of extension.rules) {
                ...
                type.prototype[rule.name] = internals.rule(joi, rule, ruleArgs, validateArgs);
            }
        }
...
internals.rule = function (joi, rule, ruleArgs, validateArgs) {
    ...
    return function (...rArgs) {
        ...
        if (rule.setup) {
            ...
            if (newSchema !== undefined) {
                Hoek.assert(Common.isSchema(newSchema), `Setup of extension ${this._type}().${rule.name}() must return undefined or a joi schema`);
                schema = newSchema;
            }
        }

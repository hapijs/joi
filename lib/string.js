...
internals.String.prototype.email = function (isEmailOptions) {
    ...
    Hoek.assert(!(isEmailOptions && isEmailOptions.checkDNS), 'checkDNS option is not supported');
    Hoek.assert(!isEmailOptions || typeof isEmailOptions.tldWhitelist === 'undefined' || typeof isEmailOptions.tldWhitelist === 'object', 'tldWhitelist must be an array or object');
    Hoek.assert(!isEmailOptions || typeof isEmailOptions.minDomainAtoms === 'undefined' || (Hoek.isInteger(isEmailOptions.minDomainAtoms) && isEmailOptions.minDomainAtoms > 0), 'minDomainAtoms must be a positive integer');
    Hoek.assert(!isEmailOptions || typeof isEmailOptions.errorLevel === 'undefined' || (Hoek.isInteger(isEmailOptions.errorLevel) && isEmailOptions.errorLevel >= 0) || typeof isEmailOptions.errorLevel === 'boolean', 'errorLevel must be a non-negative integer or boolean');

    return this._test('email', undefined, function (value, state, options) {

        if (Isemail(value, isEmailOptions)) {
            return null;
        }

...
internals.String.prototype.countryCode = function () {

    return this._test('countryCode', undefined, function (value, state, options) {

        if (isoCountries.findCountryByCode(value)) {
            return null;
        }

        return Errors.create('string.countryCode', null, state, options);
    });
};
...
internals.String.prototype.expiryYear = function () {
    ...
    var isExpired = function(year) {
        ...
        var last2 = thisYear.toString().slice(-2);
        if (year.length === 2) {
            return parseInt(year, 10) < parseInt(last2, 10) ||
                   parseInt(year, 10) > parseInt(last2, 10) +  50;
        }
        else {

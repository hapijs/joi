...
internals.String.prototype.bytes = function (min, max) {


    if (min === undefined || min === null) {
        min = 0;
    } else {
        Utils.assert(!isNaN(min) && ((min | 0) === parseFloat(min)) && min >= 0, 'minimum bytes must be a positive integer');        
    }

    if (max === undefined || max === null) {
        max = Number.MAX_VALUE;
    } else {
        Utils.assert(!isNaN(max) && ((max | 0) === parseFloat(max)) && max >= 0, 'maximum bytes must be a positive integer');
    }

    if (min > max) {
        throw new Error('minimum bytes must be smaller or equal to maximum bytes');
    }

    this._test('bytes', undefined, function (value, state, options) {

        var byteLength = new Buffer(value).length;
        if (byteLength >= min && byteLength <= max) {
            return null;
        }

        return Errors.create('string.bytes', { min: min }, state, options);
    });

...
internals.String.prototype.uri = function (uriOptions) {
    ...
    if (uriOptions) {
        ...
        Hoek.assert(/[a-zA-Z][a-zA-Z0-9+-\.]*/.test(uriOptions.scheme), 'scheme must be a valid scheme');
        regex = createUriRegex(uriOptions.scheme);
    }
...
function createUriRegex(optionalScheme) {
    ...
    // If we were passed one or more schemes, use them instead of the generic one.
    if (optionalScheme) {
        if (Array.isArray(optionalScheme)) {
            optionalScheme = optionalScheme.join('|');
        }
    ...
    var segment = pcharOnly + '*',
        ...
        path = '(' + pathAbEmpty + or + pathAbsolute + or + pathNoScheme + or + pathRootless + ')'; // Path-empty matches infinitely since it matches nothing, so it isn't represented here.

    /**
     * hier-part = "//" authority path
     */
    var hierPart = '(\\/\\/)?(' + authority + ')(' + path + ')?';

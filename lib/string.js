...
var Any = require('./any');
var JoiArray = require('./array');
var JoiDate = require('./date');
var JoiObject = require('./object');
var JoiNumber = require('./number');
...
internals.String.prototype.email = function (isEmailOptions) {
    ...
    var schema = JoiObject.keys({
        tldWhitelist: Alternatives.try(JoiObject, JoiArray),
        minDomainAtoms: JoiNumber.integer().min(0),
        errorLevel: new Any()
    }).unknown(false);
    ...
    if (isEmailOptions) {
        result = schema.validate(isEmailOptions);

        if (result.error && result.error.details) {
            message = result.error.details.map(function (detail) {

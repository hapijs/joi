    ...
    if (uriOptions) {
        ...
        if (uriOptions.scheme) {
            ...
            if (uriOptions.scheme instanceof RegExp) {
                regex = internals.createUriRegex(uriOptions.scheme.source);
            } else if (typeof uriOptions.scheme === 'string') {
                Hoek.assert(/[a-zA-Z][a-zA-Z0-9+-\.]*/.test(uriOptions.scheme), 'scheme must be a valid scheme');
                regex = internals.createUriRegex(Hoek.escapeRegex(uriOptions.scheme));
            } else {
                // With the above assertions an Array is the only thing left.
                regex = internals.createUriRegex(uriOptions.scheme.reduce(function(accumulator, scheme, index) {

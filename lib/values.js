...
module.exports = internals.Values = class {
        ...
        if (!extendedCheck) {
            if (state &&
                ...
                for (let item of this._set) {
                    if (Ref.isRef(item)) {
                        item = [].concat(item.resolve(value, state, options));
    ...
    _has(value, state, options, check) {
        ...
        for (let item of this._set) {
            if (checkRef &&
                Ref.isRef(item)) {      // Only resolve references if there is a state, otherwise it's a merge

                item = item.resolve(value, state, options);

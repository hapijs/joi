...
exports.validate = function (value, schema, state, options, reference) {
    ...
    if (schema._flags.empty &&
        !schema._flags.empty._validate(internals.trim(value, schema), null, Settings.defaults).errors) {
...
internals.finalize = function (value, schema, original, errors, state, options) {
    ...
    if (errors.length &&
        ...
        errors = internals.override(schema, errors, state, options);
    }
...
internals.override = function (schema, errors, state, options) {

    if (typeof schema._flags.error === 'function') {

        const change = schema._flags.error(errors);

        if (typeof change === 'string') {
            return [new Errors.Report('override', { reason: errors }, state, options, schema._flags, change)];
        }

        return [].concat(change).map((err) => {

            return err instanceof Error ? err :
                new Errors.Report(err.type || 'override', err.context, state, options, schema._flags, err.message, err.template);
        });
    }

    return [schema._flags.error];
};

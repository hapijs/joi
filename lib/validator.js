...
internals.tracer = function (schema, prefs) {
    ...
    if (prefs.debug) {
        Assert(Trace, 'Debug mode not supported');
...
exports.validate = function (value, schema, state, prefs) {
    ...
    if (schema._valids) {
        ...
        if (match) {
            ...
            state.mainstay.tracer.value(schema, state, 'valid', match.value);
            return internals.finalize(value, null, helpers);
        }
    ...
    if (schema._invalids) {
        ...
        if (match) {
            state.mainstay.tracer.value(schema, state, 'invalid', match.value);

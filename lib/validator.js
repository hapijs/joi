...
internals.entry = function (value, schema, prefs) {
    ...
    const tracer = schema.$_root._tracer ? schema.$_root._tracer._register(schema) : null;
    const mainstay = { externals: [], warnings: [], tracer };
...
exports.validate = function (value, schema, state, prefs) {
    ...
    state.mainstay.tracer && state.mainstay.tracer.entry(schema);
    ...
    if (def.prepare &&
        ...
        const prepared = def.prepare(schema, value, helpers);
        if (prepared) {
            if (prepared.errors) {
    ...
    if (schema._valids) {
        ...
        if (match) {
            ...
            state.mainstay.tracer && state.mainstay.tracer.value('valid', match.value, schema);
            return internals.finalize(value, null, helpers);
        }
    ...
    if (schema._invalids) {
        ...
        if (match) {
            state.mainstay.tracer && state.mainstay.tracer.value('invalid', match.value, schema);
...
internals.rules = function (value, errors, helpers) {
    ...
    for (const rule of schema._rules) {
        ...
        if (definition.convert &&
            ...
            state.mainstay.tracer && state.mainstay.tracer.log('rule', rule.name, 'full', schema);
            continue;
        }
        ...
        if (result.errors) {
            state.mainstay.tracer && state.mainstay.tracer.log('rule', rule.name, 'error', schema);
            ...
            state.mainstay.tracer && state.mainstay.tracer.log('rule', rule.name, 'value', schema);
            value = result.value;
        }
    }
...
internals.default = function (flag, value, errors, { schema, state, prefs }) {
    ...
    state.mainstay.tracer && state.mainstay.tracer.log('rule', flag, 'full', schema);

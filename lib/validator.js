...
exports.validate = function (value, schema, state, prefs) {
    ...
    if (value === undefined) {
        ...
        if (presence === 'required') {
            return internals.finalize(value, original, [schema.createError('any.required', value, null, state, prefs)], helpers);
        }
        ...
        return internals.finalize(value, original, [schema.createError('any.unknown', value, null, state, prefs)], helpers);
    }
    ...
    for (const rule of schema._rules) {
        ...
        if (rule.resolve.length) {
            ...
            for (const key of rule.resolve) {
                ...
                if (invalid) {
                    ret = schema.createError('any.ref', resolved, { arg: key, ref: args[key], reason: invalid }, state, prefs);
                    break;
                }

...
exports.validate = function (value, schema, state, prefs, reference) {
    ...
    state = state ? { key: state.key, path: state.path, ancestors: state.ancestors, flags: schema._flags } : schema._state('', [], reference !== undefined ? [reference] : []);
...
internals.finalize = function (value, schema, original, errors, state, prefs) {
    ...
    if (value !== undefined &&
        ...
        value = schema._casts[schema._flags.cast](value, original);
    }

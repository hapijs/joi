...
exports.validate = function (value, schema, state, prefs) {
    ...
    if (def.prepare &&
        ...
        if (prepared) {
            if (prepared.errors) {
                return internals.finalize(prepared.value, original, [].concat(prepared.errors), helpers);     // Prepare error always aborts early
            }
    ...
    if (def.coerce &&
        ...
        if (coerced) {
            if (coerced.errors) {
                return internals.finalize(coerced.value, original, [].concat(coerced.errors), helpers);     // Coerce error always aborts early
            }

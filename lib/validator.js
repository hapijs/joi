...
exports.validate = function (value, schema, state, options, reference) {
    ...
    if (schema._settings) {
        ...
        if (isDefaultOptions && schema._settings[Symbols.settingsCache]) {
            options = schema._settings[Symbols.settingsCache];
        }
        else {
            options = Settings.concat(schema._language ? Settings.concat({ language: schema._language }, options) : options, schema._settings);
            if (isDefaultOptions) {
                schema._settings[Symbols.settingsCache] = options;
            }
        }
    }
    ...
    if (schema._flags.empty && !schema._flags.empty._validate(schema._prepareEmptyValue(value), null, Settings.defaults).errors) {
        value = undefined;
    }
    ...
    if (presence === 'optional') {
        if (value === undefined) {
            ...
            if (isDeepDefault && schema._type === 'object') {
                value = {};
            }
            else {
    ...
    for (const test of schema._tests) {
        const ret = test.func.call(schema, value, state, options);

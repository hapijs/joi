...
internals.Object.prototype.matchKeys = function (regexp, schema) {
    this._patterns.push({
        regexp: regexp,
        schema: schema
    });
...
internals.Object.prototype.describe = function () {
    ...
    if (this._patterns.length) {
        ...
        for (var p = 0, pl = this._patterns.length; p < pl; ++p) {
            var pattern = this._patterns[p];
            var key = '/' + pattern.regexp.source + '/';
            description.children[key] = Cast.schema(pattern.schema).describe();
        }
    }
...
internals.Object.prototype._traverse = function (value, state, options) {
    ...
    if (!this._children && !this._patterns.length) {            // Object() null allows any keys
        return null;
    }
    ...
    function validateKey (key, schema) {
        var localState = { parent: value, key: key, path: (state.path ? state.path + '.' : '') + key, renamed: state.renamed };
        return Cast.schema(schema)._validate(value[key], localState, options);
    }

    for (var i = 0, il = keys.length; i < il; ++i) {
        key = keys[i];
        var child = this._children[key];
        var err = validateKey(key, this._children[key])
        if (err) {
    ...
    for (i = 0, il = this._patterns.length; i < il; ++i) {
        for (key in unprocessed) {
            var pattern = this._patterns[i];
            if (pattern.regexp.test(key)) {
                var err = validateKey(key, pattern.schema)
                if (err) {
                    errors = errors.concat(err);
                    if (options.abortEarly) {
                        return errors;

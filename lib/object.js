...
module.exports = internals.Object = function (schema) {
    ...
    Any.call(this);
    this._name = 'Object';
    this._schema = Utils.clone(schema);
...
internals.Object.prototype._traverse = function (value, state, options) {
    ...
    if (!this._schema) {            // Object() allows any keys
        return null;
    }

    var keys = Object.keys(this._schema);
    var unprocessed = Utils.mapToObject(Object.keys(value));
    var errors = [];

    for (var i = 0, il = keys.length; i < il; ++i) {
        var key = keys[i];
        var config = this._schema[key];
        var item = value[key];
    ...
    if (unprocessedKeys.length) {
        ...
        if (unprocessedKeys.length &&
            ...
            errors.push(Any.error('object.allowUnknown', { keys: unprocessedKeys.join(', ') }, state));
        }
    }

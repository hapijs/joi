...
internals.Object.prototype._base = function (value, state, options) {
    ...
    if ((this._inner.children || this._inner.patterns.length) && unprocessedKeys.length) {
        ...
        if (unprocessedKeys.length &&
            ...
            const unknownOptions = Hoek.applyToDefaults(options, { language: { label: null } }, true);
            for (let i = 0; i < unprocessedKeys.length; ++i) {
                errors.push(this.createError('object.allowUnknown', null, { key: unprocessedKeys[i], path: state.path + (state.path ? '.' : '') + unprocessedKeys[i] }, unknownOptions));
            }
        }
    }

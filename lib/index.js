        ...
        for (let i = 0; i < extensions.length; ++i) {
            ...
            if (extension.rules) {
                for (let j = 0; j < extension.rules.length; ++j) {
                    ...
                    const ruleArgs = rule.parameters ? Object.keys(rule.parameters) : [];
                    const validateArgs = rule.parameters ? Cast.schema(rule.parameters) : null;
                    ...
                    type.prototype[rule.name] = function () { // eslint-disable-line no-loop-func
                        ...
                        if (validateArgs && hasRef) {
                            validate = function (value, state, options) {

                                const expandArg = {};

                                for (let k = 0; k < ruleArgs.length; ++k) {
                                    const key = ruleArgs[k];
                                    const v = arg[key];
                                    expandArg[key] = Ref.isRef(v) ? v(state.parent, options) : v;
                                }

                                return rule.validate.call(this, expandArg, value, state, options);
                            };
                        }
                        else {
                            validate = rule.validate.bind(this, arg);
                        }

                        return this._test(rule.name, arg, validate, {
...
internals.extensionsSchema = internals.array.items(internals.object.keys({
    ...
    rules: internals.array.items(internals.object.keys({
        name: internals.string.required(),
        validate: internals.object._func().arity(4).required(),
        parameters: [

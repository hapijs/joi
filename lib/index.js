...
exports.validate = function (object, config, next) {
    ...
    var errors  = new internals.Errors();
    ...
    var processConfig = function () {
        ...
        Object.keys(unprocessedObject).forEach(function (unprocessedKey) {
            ...
            if (!skipFunctions || typeof unprocessedObject[unprocessedKey] !== 'function') {
                errors.add('the key (' + unprocessedKey + ') is not allowed');
            }
        });
    };
    ...
    var validateKeyConfig = function (keyConfig, key, value) {
        ...
        if (keyConfig instanceof Array) {
            for (var i = 0, il = keyConfig.length; i < il; ++i) {
                ...
                if (converted && keyConfig[i].validate(converted.value, key, object, errors)) {
                    errors._values = [];
                    return true;
                }
            }
        ...
        return converted && keyConfig.validate(converted.value, key, object, errors);
    };
    ...
    return errors._values.length > 0 ? next(errors.formatted()) : next();
};
...
internals.Errors.prototype.add = function (error) {

    if (error) {
        this._values.push(error);
    }
};


internals.Errors.prototype.formatted = function () {

    return new Error(this._values.join(', '));
};
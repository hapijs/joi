...
internals.root = function () {
    ...
    root.undescribe = function () {

        const described = arguments.length ? arguments[0] : any.describe();
        let schema = root[described.type]();
        Object.keys(described).map((key) => {

            switch (key) {
                case 'children':
                    const keys = {};

                    Object.keys(described.children).map((childKey) => {

                        keys[childKey] = root.undescribe(described.children[childKey]);
                    });
                    schema = schema.keys(keys);
                    break;
                case 'flags':
                    Object.keys(described.flags).map((flagKey) => {

                        switch (flagKey) {
                            case 'insensitive':
                                schema = schema.insensitive();
                                break;
                            case 'presence':
                                if (described.flags[flagKey] === 'required') {
                    ...
                    break;
                case 'valids':
                    schema = schema.valid(described.valids);
                    break;
                case 'invalids':
                    schema = schema.invalid(described.invalids);
                    break;
                case 'rules':
                    described.rules.map((item) => {

                        schema = schema[item.name](item.arg);
                    });
                    break;
                case 'items':
                    const items = [];
                    described.items.map((item) => {
                        ...
                        items.push(root.undescribe(item));
                    });

...
internals.Base = class {
    ...
    cast(to) {
        ...
        Hoek.assert(to === false || this._definition.cast[to], 'Type', this.type, 'does not support casting to', to);
    ...
    concat(source) {
        ...
        Hoek.assert(this.type === 'any' || source.type === 'any' || source.type === this.type, 'Cannot merge type', this.type, 'with another type:', source.type);
    ...
    $_addRule(options) {
        ...
        Hoek.assert(!rule.args || Object.keys(rule.args).length === 1 || Object.keys(rule.args).length === this._definition.rules[rule.name].args.length, 'Invalid rule definition for', this.type, rule.name);

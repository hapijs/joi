...
internals.Base = class {
    ...
    $_jsonSchema(mode, options = {}) {
        ...
        const allowedValues = valids && valids.filter((v) => typeof v !== 'symbol' && v !== null);
        ...
        internals.applyExamples(schema, this.$_terms.examples);
        internals.applyMetas(schema, this.$_terms.metas);
        ...
        if (this._valids) {
            ...
            if (values.length) {
                if (this._flags.only) {
                    ...
                    const extras = isTypeAny ? [] : internals.allowedValuesNeedingEnum(this, schema, rawAllowedValues, allowedValues, jsonSchemaType, prefs);
                    if (extras.length) {
                        if (!schema.anyOf) {
                            schema = {
                                anyOf: [schema]
...
internals.allowedValuesNeedingEnum = function (source, schema, rawValues, values, jsonSchemaType, prefs) {

    const base = internals.onlyBaseClone(source);
    const comparableType = jsonSchemaType || source.type;
    const extras = [];

    for (let i = 0; i < values.length; ++i) {
        const value = values[i];

        if (typeof value !== comparableType) {
            extras.push(value);
            continue;
        }

        if (base.validate(rawValues[i], prefs).error) {
            extras.push(value);
        }
    }
...
internals.jsonSchemaValues = function (source, values) {
    ...
    return values.map((value) => internals.jsonSchemaValue(source, value));
};


internals.jsonSchemaValue = function (source, value) {

    if (source.type === 'date' &&
        value instanceof Date) {

        return internals.jsonSchemaDateValue(source, value);
    }
...
internals.jsonSchemaDateValue = function (source, value) {
...
internals.applyExamples = function (schema, examples) {
    ...
    schema.examples = internals.mergeExamples(schema.examples, examples);
};


internals.applyMetas = function (schema, metas = []) {
    ...
    for (const meta of metas) {
        ...
        for (const [key, value] of Object.entries(meta)) {
            if (key === 'examples') {
                const merged = internals.mergeExamples(schema.examples, value);
                if (merged !== undefined) {

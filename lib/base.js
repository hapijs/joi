    ...
    _generate(value, state, prefs) {
        ...
        for (let i = 0; i < this.$_terms.whens.length; ++i) {
            ...
            for (let j = 0; j < tests.length; ++j) {
                ...
                const localState = state.mainstay.debug ? state.localize([...state.path, `${i}${when.switch ? '.' + j : ''}.is`], state.ancestors, is) : state.nest(is);
                if (is.$_match(input, localState, prefs)) {
                    if (then) {
                        whens.push(then._generate(value, state, prefs));
                        ids.push(`${i}${when.switch ? '.' + j : ''}.then`);
                        break;
                    }
                }
                else if (otherwise) {
                    whens.push(otherwise._generate(value, state, prefs));
                    ids.push(`${i}${when.switch ? '.' + j : ''}.otherwise`);
                    break;
                }
            }
        }

...
internals.Base = class {
    ...
    concat(source) {
        ...
        for (const test of source._tests) {
            if (!source._rules[test.method].multi) {
    ...
    extend(options) {
        ...
        prototype._rules = Object.assign({}, prototype._rules);
        ...
        if (def.rules) {
            for (const name in def.rules) {
                ...
                Hoek.assert(!prototype._rules[name], 'Rule conflict in', def.type, name);
                prototype._rules[name] = rule;
    ...
    getRule(rule) {
        ...
        return this._uniqueRules.get(rule);
    }
    ...
    _assign(target) {
        ...
        target._tests = this._tests.slice();
        target._uniqueRules = Hoek.clone(this._uniqueRules, { shallow: true });

...
internals.Base = class {
    ...
    rule(options) {
        ...
        Assert(this.$_temp.ruleset !== false, 'Cannot apply rules to empty ruleset');
    ...
    _assign(target) {
        ...
        target._cache = null;
        target._whenSources = this._whenSources && this._whenSources.slice();
        target._whenId = null;
    ...
    _generate(value, state, prefs) {
        ...
        if (this._definition.generate) {
            base = this._definition.generate(this, value, state, prefs);
        }
        ...
        if (state.mainstay.tracer.active) {
            obj._whenSources = [this];

            const each = (schema) => {

                const clone = schema.clone();
                clone._whenSources = [schema];
                return clone;
            };

            obj = obj.$_modify({ each, ref: false });
        }

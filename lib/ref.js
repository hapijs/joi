...
internals.Ref = class {
    ...
    resolve(value, state, prefs, local) {
        ...
        if (this.type === 'global') {
            return this._resolve(prefs.context);
        }

        if (this.type === 'local') {
            return this._resolve(local);
        }

        if (this.ancestor) {
            if (this.ancestor === 'root') {
                return this._resolve(state.ancestors[state.ancestors.length - 1]);
            }

            Hoek.assert(this.ancestor <= state.ancestors.length, 'Invalid reference exceeds the schema root:', this.display);
            return this._resolve(state.ancestors[this.ancestor - 1]);
        }

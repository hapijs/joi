...
internals.Number = class extends Any {
    ...
    _base(value, state, options) {
        ...
        if (!internals.isNumber(value)) {
            return { value, errors: this.createError('number.base', { value }, state, options) };
        }

        const result = { value };

        if (this._flags.precision !== undefined &&
            options.convert) {

            const precision = Math.pow(10, this._flags.precision);              // This is conceptually equivalent to using toFixed but it should be much faster
    ...
    precision(limit) {
        ...
        const obj = this._rule('precision', { args: { limit }, convert: true });

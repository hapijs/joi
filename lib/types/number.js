...
internals.float = function () {

    var isInt = internals.integer();

    return function (value, obj, key, errors, keyPath) {

        var result = (!isInt(value, obj, key, errors, keyPath));
        if (!result) {
            errors.addLocalized('number.float', key, { value: value }, keyPath);
        }
...
internals.Number.prototype.float = function () {

    this._test('float', internals.float());
    return this;
};

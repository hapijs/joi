...
const { findOrGenerate } = require('../../cache');
...
module.exports = internals.Any = class {
    ...
    _test(name, arg, func, options) {
        ...
        let obj = this.clone();
        obj = obj._addTest(name, arg, func, options);
        return obj;
    }
    ...
    _addTest(name, arg, func, options) {

        this._tests.push({ func, name, arg, options });
        if (this._flags.useCaching) {
    ...
    _useCaching() {
        ...
        clone._flags.useCaching = true;
        clone._cache = findOrGenerate(clone._createIdentifier());
        return clone;
    }
    ...
    _createIdentifier() {
        ...
        return {
            type: this._type,
            identifier: this._tests.map(({ name, arg }) => {

                return [name, arg].join('_');
            }).join()
        };
    }
    ...
    _validate(value, state, options, reference) {
        ...
        if (this._flags.useCaching) {
            const result = this._cache.get(originalValue);
            if (result) {
                return result;
            }
        }
    ...
    _finalizeValue(value, originalValue, errors, state, options) {
        ...
        if (this._flags.useCaching) {
            this._cache.set(originalValue, result);
        }

...
module.exports = internals.Any = class {
    ...
    createError(code, value, local, state, prefs, options = {}) {
        ...
        return new Errors.Report(code, value, local, options.flags !== false ? this._flags : {}, state, prefs);
    }
    ...
    static extend(options) {
        ...
        const Base = options.base || internals.Any;
        const Type = class extends Base {
            ...
            constructor(...args) {
                ...
                if (args.length) {
                    Hoek.assert(options.args, 'The', options.type, 'type does not allow arguments');

                    return options.args.call(this, ...args);
                }
            }
        };

        if (options.args) {
            Type.prototype._init = options.args;
        }

        if (options.validate) {
            Type.prototype._base = options.validate;
        }

        if (options.coerce) {
            const coerce = typeof options.coerce === 'function' ? { method: options.coerce } : options.coerce;
            Type.prototype._coerce = coerce.method;
            if (coerce.from) {
                Type.prototype._coerce.types = [].concat(coerce.from);
            }
        }

        // if (options.rules) {
        Type.prototype._rules = Object.assign({}, Type.prototype._rules);

        for (const name in options.rules) {
            const rule = options.rules[name];
            Hoek.assert(typeof rule === 'object', 'Invalid rule definition for', options.type, name);

            if (rule.method) {
                Hoek.assert(!Type.prototype[name], 'Rule conflict in', options.type, name);
                Type.prototype[name] = rule.method;
            }

            Hoek.assert(!Type.prototype._rules[name], 'Rule conflict in', options.type, name);
            Type.prototype._rules[name] = rule;

            if (rule.alias) {
                const aliases = [].concat(rule.alias);
                for (const alias of aliases) {
                    Type.prototype[alias] = rule.method;
                }
            }
        }
        // }
        ...
        if (options.overrides) {
            Type.prototype.super = internals.Any.prototype;
            Object.assign(Type.prototype, options.overrides);
        }

        if (options.cast) {
            Type.prototype._casts = Object.assign({}, Type.prototype._casts, options.cast.to);
            if (options.cast.from) {
                Type.prototype._casts[Common.symbols.castFrom] = options.cast.from;
            }
        }

        if (options.build) {
            Type.prototype._build = options.build;
        }

        if (options.modify) {
            Type.prototype._override = options.modify;
        }

        if (options.rebuild) {
            Type.prototype._rebuild = options.rebuild;
        }

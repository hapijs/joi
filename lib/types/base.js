...
internals.BaseType.prototype._conform = function (f) {

    Utils.assert(typeof f === 'function', 'In Types.Base.conform(f), the f must be a Function');
    ...
    return function (value, qs, key, req) {
      ...
      var result = f(value);
      if (result === false) {
        req.add('the value of `' + key + '` must conform to the given function');
      }
      return result;
    };
};
...
internals.BaseType.prototype.conform = function (func) {

    this.add('conform', this._conform(func), arguments);
    return this;
};

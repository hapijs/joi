...
module.exports = Any.extend({
    ...
    validate: function (schema, value, { error, state, prefs }) {
        ...
        if (!value ||
            typeof value !== schema._definition.properties.typeof ||
            Array.isArray(value)) {

            return { value, errors: error('object.base', { type: schema._definition.properties.typeof }) };
        }
    ...
    overrides: {
        ...
        default: function (value, options) {
            ...
            return this.super.default.call(this, value, options);
        },

        tailor: function (targets) {
            ...
            for (let i = 0; i < schema._inners.patterns.length; ++i) {
                ...
                for (const key of ['schema', 'rule', 'matches']) {
                    if (pattern[key] &&
                        ...
                        obj._inners.patterns[i] = Object.assign({}, pattern, { [key]: replacement });
                        return obj.rebuild();
                    }
                }
            }
    ...
    rebuild: function (schema) {
        ...
        if (schema._inners.keys) {
            ...
            for (const child of schema._inners.keys) {
                Common.tryWithPath(() => topo.add(child, { after: child.schema._refs.roots(), group: child.key }), child.key);
                schema._register(child.schema, { key: child.key });
            }

            schema._inners.keys = new internals.Keys(...topo.nodes);
        }

        if (schema._inners.patterns) {
            for (const pattern of schema._inners.patterns) {
                for (const key of ['schema', 'rule', 'matches']) {
                    schema._register(pattern[key]);
                }
            }
        }

        const assertions = schema.getRules('assert');
        for (const assertion of assertions) {
            schema._register(assertion.args.schema);
        }

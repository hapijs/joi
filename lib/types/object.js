...
internals.Object = class extends Any {
    ...
    _base(value, state, prefs) {
        ...
        if (unprocessed.size &&
            ...
            if (hasMatches) {
                const localState = this._state(state.key, state.path, ancestors);
    ...
    pattern(pattern, schema, options = {}) {
        ...
        if (options.matches) {
            ...
            obj._refs.register(config.matches);
        }
...
internals.keysToLabels = function (schema, keys) {
    ...
    const findLabel = function (key) {
        ...
        const matchingChild = schema._root.reach(schema, key);
        return matchingChild && matchingChild._getLabel() || key;
    };

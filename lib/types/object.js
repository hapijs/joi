        ...
        if (unprocessed.size &&
            ...
            if (hasMatches) {
                for (let i = 0; i < matches.length; ++i) {
                    ...
                    const localState = state.localize(state.path, ancestors, this._inners.patterns[i].matches);
        ...
        if (unprocessed.size &&
            ...
            if (forbidUnknown) {
                for (const unprocessedKey of unprocessed) {
                    ...
                    const report = this.createError('object.unknown', value[unprocessedKey], { child: unprocessedKey }, localState, prefs, { flags: false });
                    if (prefs.abortEarly) {
                        return { value, errors: report };
                    }

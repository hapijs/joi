    ...
    validate: function (value, state, prefs) {
        ...
        if (this._inners.keys) {
            ...
            for (const child of this._inners.keys) {
                ...
                const localState = state.localize([...state.path, key], ancestors, this);
        ...
        if (unprocessed.size &&
            ...
            if (hasMatches) {
                const localState = state.localize(state.path, ancestors, this);
        ...
        if (unprocessed.size &&
            ...
            if (forbidUnknown) {
                for (const unprocessedKey of unprocessed) {
                    const localState = state.localize([...state.path, unprocessedKey], [], this);
                    localState.flags = {};

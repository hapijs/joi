    ...
    rules: {
        ...
        assert: {
            method: function (ref, schema, message) {
                ...
                const obj = this.$_addRule({ name: 'assert', args: { ref, schema, message }, path });
            ...
            method: function (schema) {
                ...
                if (!schema) {                                  // Allow all
                    ...
                    for (const key in schema) {
                        Common.tryWithPath(() => obj._inners.keys.push({ key, schema: this.$_compile(schema[key]) }), key);
                    }
                }
        ...
        pattern: {
            method: function (pattern, schema, options = {}) {
                ...
                if (options.matches) {
                    ...
                    obj.$_registerChild(config.matches);
                }
    ...
    rebuild: function (schema) {
        ...
        if (schema._inners.keys) {
            ...
            for (const child of schema._inners.keys) {
                ...
                schema.$_registerChild(child.schema, { key: child.key });
            }
        ...
        if (schema._inners.patterns) {
            for (const pattern of schema._inners.patterns) {
                for (const key of ['schema', 'rule', 'matches']) {
                    schema.$_registerChild(pattern[key]);
                }
            }
        }
        ...
        for (const assertion of assertions) {
            schema.$_registerChild(assertion.args.schema);
        }

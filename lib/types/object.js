...
internals.Object = class extends Any {
    ...
    keys(schema) {
        ...
        if (!schema) {                                  // Allow all
            ...
            obj._inner.children = obj._inner.children ? obj._inner.children.filter((child) => !schema.hasOwnProperty(child.key)) : [];
            for (const key in schema) {
                internals.tryWithPath(() => obj._inner.children.push({ key, schema: Cast.schema(this._root, schema[key]) }), key);
            }
        }
    ...
    _rebuild() {
        ...
        if (this._inner.children) {
            ...
            for (const child of this._inner.children) {
                ...
                internals.tryWithPath(() => topo.add(child, { after: schema._refs.roots(), group: key }), key);
                this._register(schema, { key });
            }

            this._inner.children = topo.nodes;
        }

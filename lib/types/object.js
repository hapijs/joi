    ...
    _base(value, state, options) {
        ...
        if (this._inner.children) {
            ...
            for (let i = 0; i < this._inner.children.length; ++i) {
                ...
                const localState = new State(key, [...state.path, key], [target, ...state.ancestors]);
        ...
        if (unprocessed.size &&
            ...
            for (const key of unprocessed) {
                const localState = new State(key, [...state.path, key], [target]);
                const item = target[key];
        ...
        for (const dep of this._inner.dependencies) {
            ...
            if (failed) {
                const localState = key ? new State(key[key.length - 1], [...state.path, ...key], []) : new State(null, state.path, []);
...
internals.Object.prototype._rules = {
    ...
    assert: function (value, { error, options, state }, { schema, ref }, { key, path, message }) {
        ...
        const result = schema._validate(ref.resolve(null, { ancestors: [value, ...state.ancestors] }), null, options, value);
        if (!result.errors) {
            return value;
        }

        const localState = new State(key, ref.path, state.ancestors);

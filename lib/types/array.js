...
internals.Array = class extends Any {
    ...
    _fillMissedErrors(errors, requireds, state, options) {
        ...
        const localState = this._state(state.key, state.path);
        if (knownMisses.length) {
            if (unknownMisses) {
                errors.push(this.createError('array.includesRequiredBoth', { knownMisses, unknownMisses }, localState, options));
            }
            else {
                errors.push(this.createError('array.includesRequiredKnowns', { knownMisses }, localState, options));
            }
        }
        else {
            errors.push(this.createError('array.includesRequiredUnknowns', { unknownMisses }, localState, options));
        }
    }
...
internals.Array.prototype._rules = {
    ...
    has: function (value, { state, options, error, schema }, { schema: has }) {
        ...
        for (let i = 0; i < value.length; ++i) {
            const localState = schema._state(i, [...state.path, i], ancestors);

...
internals.Array = class extends Any {
    ...
    _rebuild() {
        ...
        for (const type of this._inner.items) {
            internals.validateSingle(type, this);

            this._register(type);

            if (type._flags.presence === 'required') {
                this._inner.requireds.push(type);
            }
            else if (type._flags.presence === 'forbidden') {
                this._inner.exclusions.push(type.optional());
            }
            else {
                this._inner.inclusions.push(type);
            }
        }

        for (const type of this._inner.ordereds) {
            internals.validateSingle(type, this);
            this._register(type);
        }

...
ArrayType.prototype._includes = function() {
    ...
    return function(values, qs, key, req) {
            ...
            for(var j = 0; j < validators.length; j++) {
                ...
                for(var m = 0; m < values.length; m++) {
                    ...
                    if (result === false) {
                        ...
                        if(self.options.debug === true) console.log(value, "is valid", currentType)
        ...
        if (valueIsValid === false) {
            ...
            var msg = "the value(s) `" + sys.inspect(invalidValues) + "` in array `" + key + "` must be of the following type(s) " + sys.inspect(validTypes);
            // req = req || {addValidationError: function(){}};
            // req.addValidationError(msg);
            // console.log(msg)

...
module.exports = Any.extend({
    ...
    coerce: {
        method: function (schema, value, { state, prefs }) {
            ...
            const sort = schema.getRule('sort');
            if (sort) {
    ...
    rules: {
        ...
        has: {
            method: function (schema) {
                ...
                const obj = this.addRule({ name: 'has', args: { schema } });
        ...
        single: {
            method: function (enabled) {
                ...
                return this.setFlag('single', value);
            }
        },
        ...
        sort: {
            method: function (options = {}) {
                ...
                return this.addRule({ name: 'sort', args: { options: settings } });
            },
        ...
        sparse: {
            method: function (enabled) {
                ...
                const obj = value ? this.clone() : this.addRule('items');
                return obj.setFlag('sparse', value, { clone: false });
            }
        },
    ...
    modify: function (schema, id, replacement) {
        ...
        for (const set of ['items', 'ordered']) {
            for (let i = 0; i < schema._inners[set].length; ++i) {
                const existing = schema._inners[set][i];
                if (id === existing._flags.id) {
                    const obj = schema.clone();
                    obj._inners[set][i] = replacement;
                    obj.rebuild();
                    return obj;
                }
            }
        ...
        const hases = schema.getRules('has');
    ...
    rebuild: function (schema) {
        ...
        schema._inners._inclusions = [];
        schema._inners._exclusions = [];
        schema._inners._requireds = [];

        for (const type of schema._inners.items) {
            internals.validateSingle(type, schema);

            schema._register(type);

            if (type._flags.presence === 'required') {
                schema._inners._requireds.push(type);
            }
            else if (type._flags.presence === 'forbidden') {
                schema._inners._exclusions.push(type.optional());
            }
            else {
        ...
        for (const type of schema._inners.ordered) {
            internals.validateSingle(type, schema);
            schema._register(type);
        }
        ...
        const hases = schema.getRules('has');
        for (const has of hases) {
            schema._register(has.args.schema);
        }
...
internals.validateSingle = function (type, obj) {
    ...
    if (type._type === 'array' ||
        ...
        obj.setFlag('_arrayItems', true, { clone: false });
    }
};

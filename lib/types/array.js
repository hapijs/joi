...
internals.Array.prototype._rules = {
    ...
    has: function (value, { state, options, error }, { schema }) {
        ...
        for (let i = 0; i < value.length; ++i) {
            const localState = new State(i, [...state.path, i], ancestors);
            if (schema._match(value[i], localState, options)) {
                return value;
            }
        }
    ...
    items: function (value, { schema, error, state, options }) {
        ...
        for (let i = 0; i < il; ++i) {
            ...
            if (!schema._flags.sparse &&
                ...
                errors.push(error('array.sparse', null, { key, path, pos: i }));
            ...
            const localState = new State(wasArray ? i : key, path, [value, ...state.ancestors]);
            ...
            for (const exclusion of schema._inner.exclusions) {
                ...
                errors.push(error('array.excludes', { pos: i, value: item }, { key, path }));
                if (options.abortEarly) {
                    return errors;
                }
            ...
            if (schema._inner.inclusions.length && !isValid) {
                ...
                errors.push(error('array.includes', { pos: i, value: item }, { key, path }));
                if (options.abortEarly) {
                    return errors;
                }
            }

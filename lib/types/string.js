...
const SPLIT_REGEX_FLAGS = /^([\/~@;%#'])(.*?)\1([igmsuy]*)$/;
const uniq = (list) => {

    return list.filter((char, pos, flagArr) => flagArr.indexOf(char) === pos);
};
    ...
    rules: {
        ...
        pattern: {
            ...
            method(regex, options = {}) {
                ...
                Assert(regex instanceof RegExp || typeof regex === 'string', 'pattern must be a RegExp or a RegExp string');

                if (typeof regex === 'string') {
                    const match = regex.match(SPLIT_REGEX_FLAGS);
                    try {
                        if (match) {
                            const [, , regexWithoutFlags, flags] = match;
                            const uniqFlags = uniq(flags.split('')).join('');

                            regex = new RegExp(regexWithoutFlags, uniqFlags);
                        }
                        else {
                        ...
                        Assert(false, 'invalid regex string');
                    }
                }

...
const internals = {
    minDomainSegments: 2,                                                                               // Match @hapi/address MIN_DOMAIN_SEGMENTS
    ...
    rules: {
        ...
        alphanum: {
            ...
            jsonSchema(rule, res) {
                ...
                return internals.appendPattern(res, '^[a-zA-Z0-9]+$');
            }
        },
        ...
        base64: {
            ...
            jsonSchema(rule, res) {
                ...
                return internals.appendPattern(res, internals.base64JsonSchemaPattern(rule.args.options));
            }
        },
        ...
        dataUri: {
            ...
            jsonSchema(rule, res) {
                ...
                return internals.appendPattern(res, internals.dataUriJsonSchemaPattern(rule.args.options));
            }
        },
        ...
        domain: {
            ...
            jsonSchema(rule, res) {
                ...
                // Not using format: 'hostname' because it accepts single-label names (e.g. 'localhost')
                // while Joi's domain() requires at least 2 segments by default

                return internals.appendPattern(res, internals.domainJsonSchemaPattern(rule.address));
            }
        },
        ...
        hex: {
            ...
            jsonSchema(rule, res, isOnly, mode) {

                return internals.appendPattern(res, internals.hexJsonSchemaPattern(rule.args.options, mode));
            }
        },
        ...
        hostname: {
            ...
            jsonSchema(rule, res) {
                ...
                return internals.appendPattern(res, internals.hostnameJsonSchemaPattern());
            }
        },
        ...
        ip: {
            ...
            jsonSchema(rule, res) {
                ...
                return internals.appendPattern(res, internals.ipJsonSchemaPattern(rule.args.options));
            }
        },
        ...
        pattern: {
            ...
            jsonSchema(rule, res) {
                ...
                return internals.appendPattern(res, rule.args.regex.source);
            },
            args: ['regex', 'options'],
            multi: true
        },
        ...
        token: {
            ...
            jsonSchema(rule, res) {
                ...
                return internals.appendPattern(res, '^[A-Za-z0-9_]+$');
            }
        },
...
internals.base64JsonSchemaPattern = function (options) {

    return internals.base64Regex[options.paddingRequired][options.urlSafe].source;
};


internals.appendPattern = function (res, pattern) {
...
internals.hostnameJsonSchemaPattern = function () {

    const hostname = internals.unanchoredPattern(internals.domainJsonSchemaPattern({ minDomainSegments: 1, tlds: false }));
    const ip = internals.unanchoredPattern(internals.ipJsonSchemaPattern({ cidr: 'forbidden' }));

    return `^(?:${hostname}|${ip})$`;
};


internals.ipJsonSchemaPattern = function (options) {

    return ipRegex(options).regex.source.replace(/\[\\w-\\\./g, '[\\w.\\-');
};


internals.dataUriJsonSchemaPattern = function (options) {

    const mediaType = 'data:[\\w+.-]+\\/[\\w+.-]+;';
    const base64 = internals.unanchoredPattern(internals.base64Regex[options.paddingRequired].false);

    return `^${mediaType}(?:base64,${base64}|(?!base64,).*)$`;
};


internals.hexJsonSchemaPattern = function (options, mode) {
...
internals.domainJsonSchemaPattern = function (options) {

    const settings = internals.addressOptions(options);

    const min = (settings.minDomainSegments || internals.minDomainSegments) - 1;
    const max = settings.maxDomainSegments !== undefined ? settings.maxDomainSegments - 1 : '';
    const fqdn = settings.allowFullyQualified ? '\\.?' : '';
    const totalLength = '(?=.{1,256}$)';
    const labelLength = '(?=[^.]{1,63}\\.)';
    const tldLength = `(?=[^.]{1,63}${settings.allowFullyQualified ? '(?:\\.?$)' : '$'})`;
    const label = internals.domainSegmentPattern({
        allowUnicode: settings.allowUnicode !== false,
        allowUnderscore: settings.allowUnderscore
    });
    const tld = internals.domainTldPattern(settings);
    const denied = internals.domainDeniedTldLookahead(settings);

    ...
    rules: {
        ...
        domain: {
            ...
            jsonSchema(rule, res) {
                ...
                res.pattern = internals.domainJsonSchemaPattern(rule.address || rule.args.options || {});
                return res;
            }
        },
...
internals.domainJsonSchemaPattern = function (options = {}) {
    ...
    const min = (options.minDomainSegments || internals.minDomainSegments) - 1;
...
internals.domainSegmentPattern = function ({ allowUnicode, allowUnderscore, tld = false, requireTwoCharacters = false }) {

    const nonAscii = allowUnicode ? '\\u0080-\\uFFFF' : '';
    const start = `[${tld ? 'A-Za-z' : `A-Za-z0-9${allowUnderscore ? '_' : ''}`}${nonAscii}]`;
    const body = `[A-Za-z0-9${nonAscii}-]`;
    const end = `[A-Za-z0-9${nonAscii}]`;

    if (requireTwoCharacters) {
...
internals.domainTldPattern = function (options = {}) {
    ...
    return internals.domainSegmentPattern({
        allowUnicode: options.allowUnicode !== false,
        tld: true,
        requireTwoCharacters: !internals.allowsAnyTld(options)
    });
};
...
internals.allowsAnyTld = function (options = {}) {
    ...
    return !internals.tldValues(options.tlds.allow);
};
...
internals.tldPatternValues = function (values, allowUnicode) {
    ...
    for (const tld of tlds) {
        patterns.add(internals.caseInsensitiveLiteral(tld));

        if (!allowUnicode) {
            continue;
        }

        const unicode = Url.domainToUnicode(tld);
        if (unicode &&
            unicode !== tld) {

            patterns.add(internals.regexLiteral(unicode));
        }
    }
...
internals.caseInsensitiveLiteral = function (value) {
    ...
    return internals.regexLiteral(value).replace(/[A-Za-z]/g, (char) => {

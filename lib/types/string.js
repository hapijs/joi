...
internals.StringType.prototype._base = function () {
    ...
    return function (value, obj, key, errors) {

        var result = typeof value === 'string';

        if (!result) {
            errors.add('the value of `' + key + '` must be a string');
        }
...
internals.StringType.prototype._min = function (n) {
    ...
    return function (value, obj, key, errors) {
        ...
        if (result === false) {
            errors.add('the value of `' + key + '` must be at least ' + n + ' characters long');
        }
        return result;
    };
};
...
internals.StringType.prototype._max = function (n) {
    ...
    return function (value, obj, key, errors) {
        ...
        var result = (value.length <= n);
        if (result === false) {
            errors.add('the value of `' + key + '` must be less than (or equal to) ' + n + ' characters long');
        }
        return result;
    };
};
...
internals.StringType.prototype._regex = function (n) {
    ...
    return function (value, obj, key, errors) {
        ...
        if (result === false) {
            errors.add('the value of `' + key + '` must match the RegExp `' + n.toString() + '`');
        }
        return result;
    };
};
...
internals.StringType.prototype._date = function () {
    ...
    return function (value, obj, key, errors) {
        ...
        if (result === false) {
            errors.add('the value of `' + key + '` must be a valid JavaScript Date format');
        }
        return result;
    };
};

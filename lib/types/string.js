    ...
    rules: {
        ...
        domain: {
            ...
            jsonSchema(rule, res) {
                ...
                const opts = rule.args.options || {};

                const underscore = opts.allowUnderscore ? '_?' : '';
                const label = `${underscore}[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?`;

                const min = (opts.minDomainSegments || internals.minDomainSegments) - 1;
                const max = opts.maxDomainSegments !== undefined ? `,${opts.maxDomainSegments - 1}` : ',';

                const tld = opts.tlds === false ? '[a-zA-Z][a-zA-Z0-9]*' : '[a-zA-Z]{2,}';
                const fqdn = opts.allowFullyQualified ? '\\.?' : '';

                res.pattern = `^(${label}\\.){${min}${max}}${tld}${fqdn}$`;
                return res;
            }
        },

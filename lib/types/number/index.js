    ...
    _base(value, state, options) {
        ...
        if (typeof value === 'string' &&
            ...
            if (!this._flags.unsafe) {
                if (value.includes('e')) {
                    const fractionDigits = value.match(/\.(\d+)e/);
                    const exponential = fractionDigits ? fractionDigits[1].length : 0;
                    if (internals.normalizeExponent(result.value.toExponential(exponential)) !== internals.normalizeExponent(value)) {

    ...
    jsonSchema(schema, res, mode, options) {
        ...
        if (schema.$_terms.keys) {
            ...
            for (const child of schema.$_terms.keys) {
                const childPrefs = child.schema._preferences
                    ? Common.preferences(prefs, child.schema._preferences)
                    : prefs;
                const presence = child.schema._flags.presence || childPrefs?.presence;
                const jsonSchema = child.schema.$_jsonSchema(mode, { ...options, ignorePresence: true });
                ...
                if (presence === 'forbidden') {
                    res.properties[child.key] = false;
                    continue;
                }

                if (mode === 'output' && child.schema._flags.result === 'strip') {
                    res.properties[child.key] = false;
                    continue;
                }
                ...
                if (presence === 'required' ||
                    (mode === 'output' && child.schema._flags.default !== undefined && !childPrefs?.noDefaults)) {

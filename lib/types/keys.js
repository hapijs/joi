...
const JsonObject = require('../json-schema/object');
    ...
    jsonSchema(schema, res, mode, options) {
        ...
        if (schema.$_terms.keys) {
            ...
            for (const child of schema.$_terms.keys) {
                const hoistedWhens = JsonObject.hoistedWhens(child.schema, child.key, mode, options, prefs);
                const childSchema = hoistedWhens ? hoistedWhens.base : child.schema;
                const childJsonSchema = JsonObject.child(childSchema, mode, options, prefs);
        ...
        if (schema.$_terms.dependencies) {
            for (const dep of schema.$_terms.dependencies) {
                JsonObject.depJsonSchema[dep.rel]?.(dep, res);
            }
        }

        for (const conditional of hoistedConditionals) {
            JsonObject.appendConditional(res, conditional);
        }

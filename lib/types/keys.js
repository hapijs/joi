...
internals.sortKeys = function (keys, manual = true) {
    ...
    for (const child of keys) {
        const childKeys = child.schema.$_terms.keys;
        const after = child.schema.$_rootReferences()
            .filter((ref) => !childKeys || !childKeys.some((local) => local.key === ref));     // Ignore references resolved internally by the child (e.g. a nested reference to a duplicated sibling key name)

        Common.tryWithPath(() => topo.add(child, { after, group: child.key, manual }), child.key);
    }

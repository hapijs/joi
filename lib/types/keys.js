...
internals.depJsonSchema = {
    ...
    with(dep, res) {
        ...
        res.dependentRequired[dep.key.key] = dep.paths;
    },
    ...
    without(dep, res) {
        ...
        res.dependentSchemas[dep.key.key] = { properties: prohibited };
    },
    ...
    and(dep, res) {
        ...
        res.dependentRequired = res.dependentRequired || {};
        for (const peer of dep.paths) {
            res.dependentRequired[peer] = dep.paths.filter((p) => p !== peer);
        }
    },
    ...
    nand(dep, res) {
        ...
        res.not = { properties: props, required: dep.paths };
    },

    or(dep, res) {

        res.anyOf = dep.paths.map((peer) => ({ properties: { [peer]: true }, required: [peer] }));
    },

    xor(dep, res) {

        res.oneOf = dep.paths.map((peer) => ({ properties: { [peer]: true }, required: [peer] }));
    },

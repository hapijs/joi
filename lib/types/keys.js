    ...
    jsonSchema(schema, res, mode, options) {
        ...
        if (schema.$_terms.keys) {
            ...
            for (const child of schema.$_terms.keys) {
                const hoistedWhens = internals.hoistedWhens(child.schema, child.key, mode, options, prefs);
                const childSchema = hoistedWhens ? hoistedWhens.base : child.schema;
                const childJsonSchema = internals.childJsonSchema(childSchema, mode, options, prefs);

                if (childJsonSchema.required) {
                    required.push(child.key);
                }

                res.properties[child.key] = childJsonSchema.schema;

                if (hoistedWhens) {
                    hoistedConditionals.push(...hoistedWhens.conditionals);
                }
            }
        ...
        if (schema.$_terms.dependencies) {
            for (const dep of schema.$_terms.dependencies) {
                internals.depJsonSchema[dep.rel]?.(dep, res);
            }
        }

        for (const conditional of hoistedConditionals) {
            internals.appendConditionalSchema(res, conditional);
        }

        // Handle 'additionalProperties' based on unknown keys flag and preferences

        if (res.additionalProperties === undefined) {
            const unknownFlag = schema._flags.unknown;

            if (unknownFlag === false) {
                res.additionalProperties = false;
            }
            else if (unknownFlag === undefined) {
                // stripUnknown: input accepts unknowns, output has them stripped

                if (prefs.stripUnknown) {
                    const stripUnknownObjects = prefs.stripUnknown === true || !!prefs.stripUnknown.objects;
                    if (mode === 'output' &&
                        stripUnknownObjects) {
                ...
                else if (!prefs.allowUnknown &&
                    (schema.$_terms.keys || schema.$_terms.patterns || schema._flags.only)) {
...
internals.depJsonSchema = {
    ...
    nand(dep, res) {
        ...
        internals.appendCompositeKeyword(res, 'not', { properties: props, required: dep.paths });
    },

    or(dep, res) {

        const branches = dep.paths.map((peer) => ({ properties: { [peer]: true }, required: [peer] }));
        internals.appendCompositeKeyword(res, 'anyOf', branches);
    },

    xor(dep, res) {

        const branches = dep.paths.map((peer) => ({ properties: { [peer]: true }, required: [peer] }));
        internals.appendCompositeKeyword(res, 'oneOf', branches);
    },

    oxor(dep, res) {

        const branches = dep.paths.map((peer) => ({ properties: { [peer]: true }, required: [peer] }));
        internals.appendCompositeKeyword(res, 'oneOf', [
            { not: { anyOf: branches } },
            ...branches
        ]);
    }
...
internals.childJsonSchema = function (schema, mode, options, prefs) {
...
internals.hoistedWhens = function (schema, key, mode, options, prefs) {
...
internals.hoistedWhen = function (base, key, when, mode, options, prefs) {
    ...
    const ref = when.ref;
    const path = internals.hoistableRefPath(ref);
    if (!path) {
    ...
    const literal = internals.literalWhenValue(when.is);
    if (!literal.found) {
        return null;
    }
...
internals.hoistedSwitchWhen = function (base, key, when, path, mode, options, prefs) {
    ...
    for (let i = when.switch.length - 1; i >= 0; --i) {
        ...
        const literal = internals.literalWhenValue(item.is);
        if (!literal.found) {
            return null;
        }
...
internals.hoistedWhenBranch = function (schema, key, mode, options, prefs) {
    ...
    const child = internals.childJsonSchema(schema, mode, options, prefs);
    const branch = {
        properties: {
            [key]: child.schema
        }
...
internals.literalWhenValue = function (schema) {
...
internals.appendConditionalSchema = function (res, conditional) {
...
internals.takeConditionalSchema = function (res) {
...
internals.promoteConditional = function (res, conditional) {
    ...
    res.allOf = [
        internals.takeConditionalSchema(res),
        conditional
    ];
};


internals.appendCompositeKeyword = function (res, keyword, value) {

    if (res.allOf) {
        if (res[keyword] !== undefined) {
            res.allOf.push({ [keyword]: res[keyword] });
            delete res[keyword];
        }

        res.allOf.push({ [keyword]: value });
        return;
    }

...
internals.Alternatives = class extends Any {
    ...
    _base(value, state, prefs) {
        ...
        for (const item of this._inner.matches) {
            ...
            if (!schema._match(input, null, prefs, state.ancestors[0])) {
                if (item.otherwise) {
    ...
    when(condition, options) {
        ...
        const normalize = (match, settings) => {
            ...
            const item = {
                ...
                then: settings.then !== undefined ? Cast.schema(this._currentJoi, settings.then) : undefined,
                otherwise: settings.otherwise !== undefined ? Cast.schema(this._currentJoi, settings.otherwise) : undefined
            };

            if (settings.is !== undefined) {
                item.is = Cast.schema(this._currentJoi, settings.is);
                if (settings.is === null ||

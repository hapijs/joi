...
module.exports = Any.extend({
        ...
        try: {
            method: function (schemas) {
                ...
                for (const schema of schemas) {
                    obj._inners.matches.push({ schema: obj.$_compile(schema) });
                }
    ...
    modify: function (schema, id, replacement) {
        ...
        let i = 0;
        for (const match of schema._inners.matches) {
            for (const key of ['schema', 'peek', 'is', 'then', 'otherwise']) {
                if (match[key] &&
                    id === match[key]._flags.id) {

                    const obj = schema.clone();
                    obj._inners.matches[i] = Object.assign({}, match, { [key]: replacement });
                    return obj.$_rebuild();
                }
            }
    ...
    rebuild: function (schema) {
        ...
        for (const match of schema._inners.matches) {
            for (const key of ['schema', 'ref', 'peek', 'is', 'then', 'otherwise']) {
                schema.$_registerChild(match[key], { family: Ref.toSibling });
            }

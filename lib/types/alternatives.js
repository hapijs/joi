...
internals.Alternatives = class extends Any {
    ...
    try(schemas) {
        ...
        for (const schema of schemas) {
            obj._inner.matches.push({ schema: Cast.schema(obj._root, schema) });
        }
    ...
    when(condition, options) {
        ...
        if (options.switch === undefined) {
            obj._inner.matches.push(normalize(condition, options));
            return obj._rebuild();
        }
        ...
        for (let i = 0; i < options.switch.length; ++i) {
            ...
            if (i + 1 !== options.switch.length) {
                ...
                obj._inner.matches.push(normalize(condition, test));
            }
            else {
                ...
                obj._inner.matches.push(normalize(condition, { is: test.is, then: test.then, otherwise }));
            }
        }

        if (options.otherwise) {
            obj._inner.matches.push(normalize(condition, { is: new Any(), otherwise: options.otherwise }));
        }

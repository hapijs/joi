...
internals.perspective = function (ref, state) {
    ...
    if (ref.type === 'local') {
        for (const schema of state.schemas) {                               // From parent to root
    ...
    if (ref.ancestor === 'root') {
        return { perspective: state.schemas[state.schemas.length - 1], path: ref.path };
    }

    return { perspective: state.schemas[ref.ancestor], path: ref.path };
};

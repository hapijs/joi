...
internals.String = class extends Any {
    ...
    email(options) {
        ...
        if (options) {
            ...
            Hoek.assert(typeof options.multiple === 'boolean', 'multiple option must be an boolean');
    ...
    lowercase() {
        ...
        const obj = this._rule('lowercase', { convert: true });
    ...
    normalize(form = 'NFC') {
        ...
        const obj = this._rule('normalize', { args: { form }, convert: true });
    ...
    trim(enabled = true) {
        ...
        let obj;
        if (enabled) {
            obj = this._rule('trim', { convert: true });
        }
        else {
            obj = this.clone();
    ...
    uppercase() {
        ...
        const obj = this._rule('uppercase', { convert: true });
...
internals.String.prototype._rules = {
    ...
    email: function (value, helpers, { options }, { regex }) {
        ...
        if (options && options.multiple) {
            emails = value.split(regex);
        }

        if (emails.every((email) => Address.email.isValid(email, options))) {
            return value;
        }

        const invalidEmails = emails.filter((email) => !Address.email.isValid(email, options)).join(', ');
        return helpers.error('string.email', { value: invalidEmails });
    },

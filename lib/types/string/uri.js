...
internals.generate = function () {
    ...
    */
    ...
    const regName = '[' + unreserved + pctEncoded + subDelims + ']{0,255}';                 // reg-name = *( unreserved / pct-encoded / sub-delims )
    ...
    */
    ...
    rfc3986.hierPart = '(?:' + '(?:\\/\\/' + authority + pathAbEmpty + ')' + '|' + pathAbsolute + '|' + pathRootless + ')';
...
internals.createRegex = function (options) {
    ...
    if (options.scheme) {
        ...
        for (let i = 0; i < schemes.length; ++i) {
            ...
            if (scheme instanceof RegExp) {
                ...
                Hoek.assert(rfc.schemeRegex.test(scheme), 'scheme at position ' + i + ' must be a valid scheme');
                selections.push(Hoek.escapeRegex(scheme));
            }
        }
    ...
    const withScheme = '(?:' + scheme + ':' + rfc.hierPart + ')';
    const prefix = options.allowRelative ? '(?:' + withScheme + '|' + rfc.relativeRef + ')' : withScheme;

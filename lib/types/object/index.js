        ...
        for (let i = 0; i < this._inner.renames.length; ++i) {
            ...
            if (rename.isRegExp) {
                ...
                for (let j = 0; j < targetKeys.length; ++j) {
                    if (new RegExp(rename.from).test(targetKeys[j])) {
                ...
                if (allUndefined) {
                    ...
                    for (let j = 0; j < matchedTargetKeys.length; ++j) {
                        const key = matchedTargetKeys[j];
                        target[internals.renameKey(key, rename.from, rename.to)] = target[key];
                    }
                }
...
internals.renameKey = function (key, from, to) {

    let groups = [];
    do {
        const match = from.exec(key);
        if (!match) {
            break;
        }
        match.shift();
        groups = groups.concat(match);
    } while (from.global);

    for (let i = 0; i < groups.length; ++i) {
        to = to.replace(new RegExp('\\$' + (i + 1), 'g'), groups[i]);
    }
    return to;
};

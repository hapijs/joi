...
internals.Array = class extends Any {
    ...
    _checkItems(items, wasArray, state, options) {
        ...
        for (let i = 0; i < il; ++i) {
            ...
            const localState = new State(key, path, state.ancestors);
            let res;
    ...
    min(limit) {
        ...
        return this._testUnique('min', limit, function (value, state, options) {
            ...
            let compareTo;
            if (isRef) {
                compareTo = limit(value, state.ancestors, options);
                    ...
                    if (records instanceof Map) {
                        ...
                        while (!(current = entries.next()).done) {
                            if (compare(current.value[0], item)) {
                                const localState = new State(state.key, [...state.path, i], state.ancestors);
                                const context = {
                                    pos: i,
                                    value: value[i],

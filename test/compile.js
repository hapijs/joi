...
describe('cast', () => {
    ...
    describe('schema()', () => {
        ...
        it('compile [null]', () => {
            ...
            expect(schema).to.equal(Joi.valid(Joi.override, null));
        });

        it('compile [1]', () => {

            const schema = Joi.compile([1]);
            expect(schema).to.equal(Joi.valid(Joi.override, 1));
        });

        it('compile ["a"]', () => {

            const schema = Joi.compile(['a']);
            expect(schema).to.equal(Joi.valid(Joi.override, 'a'));
        });
        ...
        it('compile [1, 2, 3]', () => {
            ...
            expect(schema).to.equal(Joi.valid(Joi.override, 1, 2, 3));
        });
        ...
        it('compile [null, "a", 1, true]', () => {
            ...
            expect(schema).to.equal(Joi.valid(Joi.override, null, 'a', 1, true));
        });
    });
